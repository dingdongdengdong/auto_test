...
"""
Fast-route pure pursuit controller node.

This node is started by `pure_pursuit/launch/faster_pure_pursuit.launch` and
controls the vehicle using a pure pursuit algorithm with a more aggressive
mission timing and speeds tailored for the "faster" course variants
(`first_faster`, parking segments, then `second_faster`).

Key interfaces
- Subscribes
  - `/gps` (morai_msgs/GPSMessage): Vehicle latitude/longitude
  - `/imu` (sensor_msgs/Imu): Orientation used to compute yaw
  - `/traffic_light` (std_msgs/Int64MultiArray): [red_count, green_count]
  - `/cam_steer` (morai_msgs/CtrlCmd): Camera/lane-based steering override
  - `/curve_cmd` (morai_msgs/CtrlCmd): Steering/velocity override for curve mission
  - `/dy_obs_info` (std_msgs/Float64MultiArray): Dynamic obstacle info [dist, lateral, ...]

- Publishes
  - `/global_path` (nav_msgs/Path): Global route visualization
  - `/ctrl_cmd` (morai_msgs/CtrlCmd): Longitudinal/lateral control commands
  - `/mission` (std_msgs/Bool): Mission stage marker
  - `/waypoint` (std_msgs/Int64): Current closest waypoint index

- Service clients
  - `/Service_MoraiEventCmd` (morai_msgs/MoraiEventCmdSrv): Gear/indicator/emergency lights

Main flow
- Load path text from `path_maker/path/*.txt`
- Loop at 20 Hz: compute local path, determine mission segment, compute steering
  via pure pursuit, then publish `/ctrl_cmd`.

Note
- Geographic coords are converted to UTM (zone 52) for internal planning.
"""
...
class PurePursuit():
    def __init__(self):
        """Initialize publishers/subscribers, service client, and mission state.

        The default course is `first_faster`. The node coordinates several
        missions such as start/stop on slope, right-angle course, S-curve,
        parking (T mission), dynamic obstacle handling, traffic lights, and
        final acceleration sections. All speed/lfd envelopes are tuned for the
        "faster" variant.
        """
        ...
        self.path_name = 'clean_simple_L_course'
...
##############################################################################################################################################################
    ...
    def forward_mode(self):
        """Set gear to Drive and turn off turn/emergency signals."""
        self.req.option = 6
        self.req.gear = 4
    ...
    def rear_mode(self):
        """Set gear to Reverse and mark yaw as rear for rear-steering mode."""
        self.req.option = 2
        self.req.gear = 2
        response = self.req_service(self.req)
        self.yaw_rear = True
    ...
    def drive_left_signal(self):
        """Enable left turn indicator while staying in Drive."""
        self.req.option = 6
        self.req.gear = 4
    ...
    def drive_right_signal(self) :
        """Enable right turn indicator while staying in Drive."""
        self.req.option = 6
        self.req.gear = 4
    ...
    def emergency_mode(self) :
        """Enable emergency warning lights."""
        self.req.option = 6
        self.req.gear = 4
    ...
    def brake(self) :
        """Full brake command; zero velocity/steer."""
        self.ctrl_cmd_msg.longlCmdType = 2
        self.motor_msg = 0.0
        self.servo_msg = 0.0
        self.brake_msg = 1.0
...
###################################################################### Call Back ######################################################################
    ...
    def gpsCB(self, msg): 
        """Handle GPSMessage, store lat/lon, convert to UTM, update odometry."""
        #UTMK
    ...
    def convertLL2UTM(self) :
        """Convert current WGS84 lat/lon to UTM (zone 52)."""
        xy_zone = self.proj_UTM(self.lon, self.lat)
        self.x, self.y = xy_zone[0], xy_zone[1]

    def ImuCB(self, msg) :
        """Store orientation from IMU and compute yaw in degrees."""
...
###################################################################### Function ######################################################################
    ...
    def setMotorMsgWithVel(self, velocity):
        """Utility: set target velocity field (m/s or sim-specific units)."""
        self.motor_msg = velocity

    def setServoMsgWithLfd(self, lfd):
        """Compute steering with given look-ahead distance and set servo field."""
