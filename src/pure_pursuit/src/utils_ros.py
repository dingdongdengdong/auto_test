...
def findLocalPath(ref_path, vehicle_longitude, vehicle_latitude):
    out_path=  Path()
    current_x = vehicle_longitude
    current_y = vehicle_latitude
    current_waypoint = 0
    min_dis = float('inf')
    ...
    def curveBasedVelocity(self,global_path,point_num):
        ...
        for i in range(point_num,len(global_path.poses)-point_num):
            ...
            for box in  range(-point_num,point_num):
                x = global_path.poses[i+box].pose.position.x#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
class purePursuit :
    def __init__(self):
        ...
        rospy.Subscriber("/yaw", Float64, self.yawCB) ## Vehicle Yaw Subscriber 
