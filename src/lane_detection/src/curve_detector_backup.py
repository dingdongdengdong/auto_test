...
class Controller() :
    ...
    def image_callback(self, _data) :
        ...
        if self.original_longitude  <= 1 and self.original_latitude <= 1 and self.mission == False:
            ...
            self.slide_img, self.slide_x_location, self.current_lane_window = self.slidewindow.slidewindow(warper_image, self.steering_val)
            self.curve_img, self.angle = self.slidewindow.curve(warper_curve)
