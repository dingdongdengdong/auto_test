...
class SlideWindow:
    ...
    # 곡선/커브 방향 추정을 위한 ROI 선분 각도 계산
    # 입력: 탑뷰 이진 이미지
    # 반환: ROI 이미지, 대표 선분 각도(도)
    def curve(self, img) :
        ## 수평선 상자
        curve_img = img[380:470, 230:300] # 390 / 240
    ...
    # 슬라이딩 윈도우로 차선 중심 x 좌표 계산
    # 입력: 탑뷰 이진 이미지(img), 차량 yaw(도)
    # 반환: 시각화 이미지(out_img), 차선 중심 x 좌표(x_location), 현재 추종 차선(LEFT/RIGHT/MID)
